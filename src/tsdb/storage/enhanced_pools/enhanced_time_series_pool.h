#ifndef TSDB_STORAGE_ENHANCED_TIME_SERIES_POOL_H_
#define TSDB_STORAGE_ENHANCED_TIME_SERIES_POOL_H_
...
/**
 * @brief Enhanced TimeSeriesPool with cache alignment and bulk operations
 * 
 * This enhanced version of TimeSeriesPool provides:
...
public:
     ...
     */
    explicit EnhancedTimeSeriesPool(size_t initial_size = 100, size_t max_size = 10000);
    
    /**
     * @brief Destructor
     */
    ~EnhancedTimeSeriesPool();
    
    /**
     * @brief Acquire a cache-aligned TimeSeries object from the pool
     * @return Unique pointer to a cache-aligned TimeSeries object
     */
    std::unique_ptr<core::TimeSeries> acquire_aligned();
    
    /**
     * @brief Acquire multiple TimeSeries objects in bulk
     * @param count Number of objects to acquire
     * @return Vector of unique pointers to TimeSeries objects
     */
    std::vector<std::unique_ptr<core::TimeSeries>> acquire_bulk(size_t count);
    
    /**
     * @brief Release multiple TimeSeries objects in bulk
     * @param objects Vector of unique pointers to objects to release
     */
    void release_bulk(std::vector<std::unique_ptr<core::TimeSeries>>& objects);
...
private:
    ...
    static constexpr size_t CACHE_ALIGNED_BLOCK_SIZE = 1024; // 1KB cache-aligned blocks
...
#endif // TSDB_STORAGE_ENHANCED_TIME_SERIES_POOL_H_
